 ...
 * @return array
 */
function directoria_body_classes( $classes ) {
...
add_filter( 'body_class', 'directoria_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function directoria_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'directoria_pingback_header' );

// Show breadcrumb
function the_directoria_breadcrumb() {
    ...
    if (is_category() || is_single()) {
        echo '<li class="bread_active">'. get_the_category_list(' &bull; ') .'</li>';
    } elseif (is_single()) {
        echo '<li>'. the_category(' &bull; ') .'</li>';
        echo '<li class="bread_active">'. get_the_title() .'</li>';

    } elseif (is_page()) {
        echo '<li class="bread_active">'. get_the_title() .'</li>';
    } elseif (is_search()) {
        echo '<li class="bread_active">Search Results for... ';
        echo '"<em>';
        the_search_query();
        echo '</em>"</li>';
    }
...
// Breadcrumbs
function directoria_adv_breadcrumbs() {
    ...
    // show the heading first and then breadcrumb
    if (is_front_page()){
        echo  '<h2>'.__('Blog', 'directoria').'</h2>';
    }elseif (is_single()){
        the_title('<h2>','</h2>');
    }elseif (is_archive()){
        ...
        $s = get_search_query();
        // if the search query is empty for at_biz_dir post type then show 'All' string instead of an empty string in the breadcrumb heading in Directoria theme
        if (empty($s)){
            $at_biz_dir = get_query_var('post_type');
    ...
    if ( !is_front_page() ) {
        ...
        if ( is_archive() && !is_tax() && !is_category() && !is_tag() && !is_author() && !is_day() && !is_year() && !is_month() ) {
            ...
            echo '<li class="breadcrumb-item active"><strong>' . post_type_archive_title('', false) . '</strong></li>';
            ...
            // Paginated archives
            echo '<li class="breadcrumb-item active item-current item-current-' . get_query_var('paged') . '"><strong class="bread-current bread-current-' . get_query_var('paged') . '" title="Page ' . get_query_var('paged') . '">'.__('Page', 'directoria') . ' ' . get_query_var('paged') . '</strong></li>';

        } else if ( is_search() ) {

            // Search results page
            echo '<li class="breadcrumb-item active item-current item-current-' . get_search_query() . '"><strong class="bread-current bread-current-' . get_search_query() . '" title="Search results for: ' . get_search_query() . '">Search</strong></li>';
